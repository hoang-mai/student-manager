...
export default function Main() {
  ...
  const { data: profileResponse, isLoading, isError, refetch, error } = useQuery({
  ...
  if (!profile || !profile.profile) {
    return <ErrorState message="Không tìm thấy thông tin hồ sơ" onRetry={() => refetch()} />;
  }
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div className="flex flex-col items-center md:items-start gap-3 text-center md:text-left">
            <div className="flex flex-col md:flex-row items-center gap-3">
              <Typography variant="h1" weight="black" transform="uppercase" className="tracking-tight leading-none">
            ...
            <Typography variant="body" color="gray" className="flex items-center gap-2 font-medium">
              <HiOutlineIdentification size={16} className="text-neutral-400" />
              Mã chỉ huy: <span className="font-bold text-neutral-800">{commander?.code || "---"}</span>
            </Typography>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          <section className="bg-white/40 backdrop-blur-md border border-white/60 rounded-[32px] p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <InfoItem icon={HiOutlineLocationMarker} label="Quê quán" value={commander?.hometown} />
              <InfoItem icon={HiOutlineLocationMarker} label="Nơi sinh" value={commander?.placeOfBirth} />
              <InfoItem icon={HiOutlineGlobeAlt} label="Dân tộc" value={commander?.ethnicity} />
              <InfoItem icon={HiOutlineLibrary} label="Tôn giáo" value={commander?.religion} />
              <InfoItem icon={HiOutlineLocationMarker} label="Địa chỉ hiện tại" value={commander?.currentAddress} />
            </div>
          </section>
          ...
          <section className="bg-white/40 backdrop-blur-md border border-white/60 rounded-[32px] p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <InfoItem icon={HiOutlineCalendar} label="Ngày nhập ngũ" value={formatDateTime(commander?.dateOfEnlistment)} />
              <InfoItem icon={HiOutlineClock} label="Thâm niên (Năm)" value={commander?.startWork} />
            </div>
          </section>
        </div>
        ...
        <div className="flex flex-col gap-8">
          <section className="bg-white/40 backdrop-blur-md border border-white/60 rounded-[32px] p-8 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-2 h-8 bg-primary-500 rounded-full" />
              <Typography variant="h3" weight="black" transform="uppercase">Thông tin Đảng viên</Typography>
            </div>

            <div className="flex flex-col gap-4">
              <InfoItem icon={HiOutlineShieldCheck} label="Số thẻ Đảng" value={commander?.partyMemberCardNumber} />
              <InfoItem icon={HiOutlineCalendar} label="Ngày vào Đảng (Dự bị)" value={formatDate(commander?.probationaryPartyMember)} />
              <InfoItem icon={HiOutlineCalendar} label="Ngày vào Đảng (Chính thức)" value={formatDate(commander?.fullPartyMember)} />
            </div>
          </section>
          ...
          <section className="bg-white/40 backdrop-blur-md border border-white/60 rounded-[32px] p-8 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-2 h-8 bg-secondary-500 rounded-full" />
              <Typography variant="h3" weight="black" transform="uppercase">Dữ liệu hệ thống</Typography>
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center p-3 rounded-xl hover:bg-white/50 transition-colors">
                <Typography variant="body" color="gray" className="font-medium">Ngày tham gia</Typography>
                <Typography variant="body" weight="bold" className="text-neutral-800">{formatDateTime(profile?.createdAt)}</Typography>
              </div>
              <div className="w-full h-px bg-neutral-100" />
              <div className="flex justify-between items-center p-3 rounded-xl hover:bg-white/50 transition-colors">
                <Typography variant="body" color="gray" className="font-medium">Cập nhật cuối</Typography>
                <Typography variant="body" weight="bold" className="text-neutral-800">{formatDateTime(profile?.updatedAt)}</Typography>
              </div>
            </div>
          </section>
        </div>
