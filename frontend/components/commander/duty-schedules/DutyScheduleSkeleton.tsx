...
export default function DutyScheduleSkeleton() {
  const columnCount = 8;
  return (
    <div className="space-y-8 rounded-2xl bg-white p-6 min-h-screen">
      ...
      <div className="flex items-center gap-2 text-neutral-400">
        ...
        <Typography variant="label" color="primary" tracking="wide">Phân công lịch trực</Typography>
      </div>

      {/* Header */}
      <div className="relative flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div>
          <Typography variant="h1" transform="uppercase">Phân công lịch trực</Typography>
        </div>
      </div>
                  ...
                  <tbody>
                    {Array.from({ length: 10 }).map((_, rowIdx) => (
                      <tr key={rowIdx} className="border-t border-neutral-50">
                        ...
                        {/* Nhiệm vụ */}
                        <td className="px-4 py-3"><Skeleton variant="rounded" width={80} height={20} /></td>
                        {/* Số điện thoại */}
                        <td className="px-4 py-3"><Skeleton variant="text" width={100} height={14} /></td>
                        {/* Ngày trực */}
                        <td className="px-4 py-3"><Skeleton variant="text" width={90} height={14} /></td>
                        {/* Ngày tạo */}
